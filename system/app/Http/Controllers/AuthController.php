...
class AuthController extends Controller{
	...
	function loginProcess(){
		if(Auth::attempt(['email' => request('email'),'password' => request('password')])){
			...
			//level pengguna
			$user = Auth::user();
			if($user->level == 1) return redirect('beranda/penjual')->with('success','Login Berhasil');
			if($user->level == 0) return redirect('beranda/pembeli')->with('success','Login Berhasil');
		}else{
